...
ExpandedPanel.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
ExpandedPanel.BorderSizePixel = 0
ExpandedPanel.Position = UDim2.new(0, 0, 0, 45)
ExpandedPanel.Size = UDim2.new(0, 240, 0, 400)
ExpandedPanel.Visible = false
ExpandedPanel.Parent = MainFrame

local UIListLayout = Instance.new("UIListLayout")
UIListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
UIListLayout.Padding = UDim.new(0, 6)
UIListLayout.Parent = ExpandedPanel
...
TitleLabel.InputBegan:Connect(function(input, gameProcessed)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 then
		...
		ExpandedPanel.Visible = isExpanded
		MainFrame.Size = isExpanded and UDim2.new(0, 240, 0, 445) or UDim2.new(0, 240, 0, 45)
	end
end)
...
local function setupButton(text, order, color, customParent)
	...
	btn.Parent = customParent or ExpandedPanel
	return btn
end
...
DummyFrame.Parent = ExpandedPanel
...
SpeedFrame.Parent = ExpandedPanel
...
DropdownContainer.Parent = ExpandedPanel
...
ActionRowFrame.Parent = ExpandedPanel
...
PlayBtn.MouseButton1Click:Connect(function()
	local char = LocalPlayer.Character
	local root = char and char:FindFirstChild("HumanoidRootPart")
	if not char or not root or isPlaying or not savedData or next(savedData) == nil then 
		print("Cannot play: Character missing, already playing, or no saved data")
		return 
	end
	...
	task.spawn(function()
		repeat
			...
			print("Animation duration: " .. totalAnimDuration)
				...
				if dataA and dataB then
					...
					if char and root and root.Parent then
						for jName, jointInstance in pairs(myJoints) do
							...
							if tA and tB and jointInstance then
								pcall(function()
									jointInstance.Transform = tableToCf(tA):Lerp(tableToCf(tB), alpha)
								end)
							elseif tA and jointInstance then
								pcall(function()
